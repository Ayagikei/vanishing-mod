...
#ifndef CLIENT_DLL
// For auto lowering sound volume. See the end of this file.
#include "ai_speech.h"
#endif
...
CON_COMMAND_F(playerspeak, "Emit specific sound.\n\tArguments: <sound name> <volume> <pitch>\n", FCVAR_GAMEDLL) {
	...
	float volume = (float)Q_atof(args.Arg(2));
	int pitch = Q_atoi(args.Arg(3));
...
#define RANDOMIZE_PITCH 0
		...
		if (volume > 0.05) {
			// HACK: we need to know how to trigger this on client side.
			engine->ClientCommand(engine->PEntityOfEntIndex(player->entindex()), "playerspeak %s %f %d", soundname, volume, pitch);
		}
#endif
	}
#ifdef _DEBUG
...
#endif
			;
		Warning("UTIL_PlayerSpeak: local player is not found. (%s)", side);
	}
#endif
}